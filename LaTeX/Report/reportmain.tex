...
\todo[inline]{Move introduction of lambda parameters in this section}
...
Let $g_0^{(i)}$ and $g_1^{(i)}$ be the generating functions of respectively the degree and the excess degree distribution in layer $i$. Moreover define $u_i$ as the probability that a vertex reached after following an edge in layer $i$ is not part of the giant viable cluster. Be aware that $u_i$ is not the direct extension of the single layer $u$ quantity, which correspond to the probability not to be part of the GCC.\todo{Whaaat ?}
...
Moreover, per definition of the generating functions \eqref{Definition of g0} and \eqref{Definition of g1}\todo{Verify ref}, we have $g^{(i)}_0(1) = 1$ and $g^{(i)}_1(1) = 1$ for all layers. As a consequence, eq. \eqref{Multiplex u final} always admits the trivial solution
\begin{align}
...
\end{align}
If this equation only admits the trivial solution $\uvec_T = (1, 1, \dots, 1)$, the parameters $\lambdavec$ correspond to a state without GVC. On the other hand if multiple solutions $\uvec$ exist, a GVC must exist as well. To determine the boundary between these two regions, that we call the \newconcept{critical region} $\mathcal{R}$, we use the implicit function theorem.
...
\begin{figure}
	...
	\todo[inline]{Do other than ER, put label with number of layers add simulations to plot}
	\todo[inline]{Write corresponding equation, reference figure somewhere, put it somewhere better}
\end{figure}
